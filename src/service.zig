...
fn erlsrv(allocator: Allocator, command: []const u8, rel: Release) !void {
    ...
    // const argv = try erlsrvArgs(allocator, command, erlsrv_path, rel);
    const erl_srv_command = std.meta.stringToEnum(ErlSrvCommand, command) orelse fatal("failed to create ErlSrvCommand enum from {s}", .{command});
    const argv = switch (erl_srv_command) {
        .add => &[_][]const u8{
            erlsrv_path,
            "add",
...
fn erlsrvPath(allocator: Allocator, rel: Release) ![]const u8 {
    ...
    if (pathExists(erts_path))
        return try fs.path.join(allocator, &[_][]const u8{ erts_path, "bin", "erlsrv.exe" });
    return "erlsrv.exe";
}
...
//     const add_args = erlsrvArgs(allocator, "add", erlsrv_path, rel);
//     const remove_args = erlsrvArgs(allocator, "remove", erlsrv_path, rel);
//     const start_args = erlsrvArgs(allocator, "start", erlsrv_path, rel);
//     const stop_args = erlsrvArgs(allocator, "stop", erlsrv_path, rel);
//     const enable_args = erlsrvArgs(allocator, "enable", erlsrv_path, rel);
//     const disable_args = erlsrvArgs(allocator, "disable", erlsrv_path, rel);
//     const list_args = erlsrvArgs(allocator, "list", erlsrv_path, rel);
//     const help_args = erlsrvArgs(allocator, "help", erlsrv_path, rel);
